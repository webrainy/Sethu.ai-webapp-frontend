...
import ManageInterviewsModal from "../../components/modal/admin/ManageInterviewsModal";
...
function AdminManageInterviews() {
  ...
  useEffect(() => {
    ...
    dispatch(
      listEventItem({
        end_point: `/api/event/list?event_type=2`,
        access_token: access_token,
      })
    ).unwrap();
  }, [dispatch]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!data.batch_name) {
      ...
      urlencoded.append("event_type", "2");
  ...
  //   const handleUpdateModal = async (interview_data) => {
  ...
  //   const handleInterviewDelete = async (interview_data) => {
    ...
    <>
      <div className="p-3">
        <div className="flex justify-between items-center">
          <p className="text-3xl font-ddin font-semibold">Manage Interviews</p>
            ...
            {event_items.length > 0 ? (
              <div className="flex flex-col gap-3 mt-4">
                ...
                <LuPencil
                  ...
                  onClick={() => handleUpdateModal(interview)}
                />
                <MdOutlineDeleteForever
                  className="text-2xl cursor-pointer"
                  onClick={() => handleInterviewDelete(interview)}
                />
              </div> */}
                    <div className="flex justify-between items-start gap-4">
                      ...
                      <p className="font-myriad text-sm font-light text-gray-600">
                        {/* Feb, 08 2025 12.00pm */}
                        {moment(interview.datetime).format("LLL")}
                      </p>
                    </div>
                    <div className="flex justify-between items-end gap-4">
                      ...
                      <Button
                        onClick={() => window.open(interview.url)}
...
export default AdminManageInterviews;
